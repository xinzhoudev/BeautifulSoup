...
def find_p_and_replace_class(filename):
    ...
    def xml(tag, attrs):
        if tag == "name":
            attrs["class"] = "test"
        return tag
    rep_xml = SoupReplacer(attrs_xformer=xml)
    ...
    name_without_ext = os.path.splitext(filename)[0] + "_with_class"
    if(filename.lower().endswith(".html")): name_without_ext += ".html"
    elif(filename.lower().endswith(".xml")): name_without_ext += ".xml"
    with open(name_without_ext, "w", encoding = "utf-8") as f:
